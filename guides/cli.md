# CLI Tools

The library provides Mix tasks for working with BPMN files from the command line.

## `mix rodar.validate`

Validate a BPMN 2.0 XML file for structural issues:

```shell
mix rodar.validate path/to/process.bpmn
```

Runs 9 structural validation rules on each process:

- Start/end event existence and connectivity
- Sequence flow reference integrity
- Orphan node detection
- Gateway outgoing flow counts
- Exclusive gateway default flow (warning)
- Boundary event attachment

If a collaboration element is present, cross-process constraints are also checked (participant refs, message flow refs).

Exit code 0 on clean or warnings-only, exit code 1 on errors.

## `mix rodar.inspect`

Print the parsed structure of a BPMN file:

```shell
mix rodar.inspect path/to/process.bpmn
```

Output includes:

- Diagram ID
- Each process with element counts grouped by type
- Element IDs for each type
- Collaboration info (participants, message flows) if present

## `mix rodar.run`

Execute a BPMN process from an XML file:

```shell
mix rodar.run path/to/process.bpmn
mix rodar.run path/to/process.bpmn --data '{"username": "alice"}'
mix rodar.run path/to/process.bpmn --non-interactive
```

Starts the application, parses the BPMN file, and runs the first process. The task automatically attempts convention-based handler auto-discovery, looking for handler modules at the paths generated by `mix rodar.scaffold` (e.g., `MyApp.Workflow.OrderProcessing.Handlers.ValidateOrder`). Any discovered handlers are wired automatically; tasks without handlers use a passthrough that lets execution continue.

The header output shows which handlers were discovered and which are using passthrough:

```
  Discovered handlers:
    - Task_Validate → MyApp.Workflow.OrderProcessing.Handlers.ValidateOrder
  Service tasks without handlers (will pass through):
    - Fulfill Order (Task_Fulfill)
```

The `--data` flag accepts a JSON object that is passed as initial data to the process context.

The `--non-interactive` flag skips user task prompts (useful for CI/testing).

## `mix rodar.export`

Export a BPMN file as normalized BPMN 2.0 XML:

```shell
mix rodar.export path/to/process.bpmn
mix rodar.export path/to/process.bpmn --output normalized.bpmn
```

Parses the input file and re-exports it as normalized BPMN 2.0 XML. This is useful for:

- Normalizing XML formatting across different BPMN editors
- Stripping vendor-specific extensions (e.g., Camunda, Drools attributes)
- Verifying round-trip fidelity of the parser

Prints to stdout by default. Use `--output` to write to a file instead.

## `mix rodar.scaffold`

Generate handler module stubs from a BPMN file:

```shell
mix rodar.scaffold path/to/order.bpmn
mix rodar.scaffold path/to/order.bpmn --output-dir lib/my_app/handlers
mix rodar.scaffold path/to/order.bpmn --module-prefix MyApp.Handlers
mix rodar.scaffold path/to/order.bpmn --dry-run
mix rodar.scaffold path/to/order.bpmn --force
```

Parses the BPMN file, identifies all actionable tasks (service, user, send, receive, manual, and generic), and generates handler module files with the correct behaviour and callback stubs.

### Options

| Flag | Alias | Description |
|------|-------|-------------|
| `--output-dir DIR` | `-o` | Override the default output directory |
| `--module-prefix PREFIX` | `-p` | Override the derived module prefix |
| `--dry-run` | `-d` | Print generated code to stdout instead of writing files |
| `--force` | `-f` | Overwrite existing files without prompting |

### Defaults

- **Output directory**: `lib/<app_name>/workflow/<bpmn_filename>/handlers/`
- **Module prefix**: `<AppName>.Workflow.<BpmnFilename>.Handlers`

The namespace segment (`Workflow`) is configurable via `config :rodar, :scaffold_namespace`.

### Task type mapping

- Service tasks (`:bpmn_activity_task_service`) get the `Rodar.Activity.Task.Service.Handler` behaviour with an `execute/2` callback
- All other task types (user, send, receive, manual, generic) get the `Rodar.TaskHandler` behaviour with a `token_in/2` callback

### Conflict handling

When a target file already exists and `--force` is not set, the task shows a diff and prompts:

- **Overwrite** — replace the existing file
- **Keep both** — write a new file with a `New` suffix
- **Skip** — leave the existing file unchanged

After writing files, the task prints registration instructions showing how to wire the generated handlers (via `handler_map` for service tasks, or `TaskRegistry` for others).

### Convention-Based Auto-Discovery

Scaffolded handlers are placed at deterministic module paths following the convention `<AppName>.Workflow.<BpmnFilename>.Handlers.<TaskName>`. This means after scaffolding, you can use auto-discovery instead of manual wiring:

```elixir
# Instead of manually building a handler_map, just load with discovery:
diagram = Rodar.Engine.Diagram.load(xml,
  bpmn_file: "order_processing.bpmn",
  app_name: "MyApp"
)
# All scaffolded handlers at MyApp.Workflow.OrderProcessing.Handlers.* are found automatically
```

`mix rodar.run` also uses discovery automatically, so after scaffolding and implementing your handlers, re-running the process will pick them up without any registration code.

## Next Steps

- [Getting Started](getting_started.md) — Installation and basic concepts
- [Process Lifecycle](process_lifecycle.md) — Instance creation and status transitions
