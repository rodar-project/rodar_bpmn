defmodule RodarBpmn.HooksTest do
  ...
  describe "register/3 and notify/3" do
    test "before_node hook is called with correct metadata" do
      test_pid = self()
      {:ok, context} = RodarBpmn.Context.start_link(simple_process(), %{})
      ...
      RodarBpmn.execute(start, context)
    ...
    test "on_error hook fires on error end event" do
      test_pid = self()
      {:ok, context} = RodarBpmn.Context.start_link(error_process(), %{})
  ...
  describe "no hooks registered" do
    test "no crash when no hooks are registered" do
      {:ok, context} = RodarBpmn.Context.start_link(simple_process(), %{})
      start = simple_process()["start"]
      assert {:ok, ^context} = RodarBpmn.execute(start, context)
    end
  end
