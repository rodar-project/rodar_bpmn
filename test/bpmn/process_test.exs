defmodule Bpmn.ProcessTest do
  ...
  describe "start_link/2" do
    test "creates a process instance in :created status" do
      ...
      assert Bpmn.Process.status(pid) == :created
    end

    test "returns error for unregistered process" do
      Process.flag(:trap_exit, true)
      assert {:error, _} = Bpmn.Process.start_link("nonexistent")
    end
  end
  ...
  describe "activate/1" do
    ...
    test "cannot activate a non-created process" do
      ...
      assert {:error, _} = Bpmn.Process.activate(pid)
    end
  end
  ...
  describe "create_and_run/2" do
    ...
    test "returns error for unregistered process" do
      assert {:error, _} = Bpmn.Process.create_and_run("nonexistent")
    end
  end
  ...
  describe "suspend/1 and resume/1" do
    test "suspend/resume status transitions" do
      ...
      assert {:error, _} = Bpmn.Process.suspend(pid)
    end
    ...
    test "cannot resume a non-suspended process" do
      ...
      assert {:error, _} = Bpmn.Process.resume(pid)
    end
  end
  ...
  describe "terminate/1" do
    test "terminates and stops context" do
      ...
      assert :ok = Bpmn.Process.terminate(pid)
      assert Bpmn.Process.status(pid) == :terminated
  ...
  describe "instance_id/1" do
    test "returns a unique instance ID" do
      register_simple_process()
      {:ok, pid1} = Bpmn.Process.start_link(@process_id)
      {:ok, pid2} = Bpmn.Process.start_link(@process_id)

      assert Bpmn.Process.instance_id(pid1) != Bpmn.Process.instance_id(pid2)
    end
  end
