defmodule Bpmn.TaskHandlerTest do
  ...
  defmodule CountingHandler do
    ...
    def token_in({_type, %{id: id}}, context) do
      Bpmn.Context.put_data(context, "handled_by", id)
      {:ok, context}
    end
  end
  ...
  defmodule TypeHandler do
    ...
    def token_in({type, _attrs}, context) do
      Bpmn.Context.put_data(context, "handler_type", Atom.to_string(type))
      {:ok, context}
    end
  end
  ...
  describe "custom handler by type" do
    test "handler is invoked for registered type" do
      ...
      {:ok, ^context} = Bpmn.execute(start, context)
      assert Bpmn.Context.get_data(context, "handler_type") == "my_custom_task"
    end
  end
  ...
  describe "custom handler by task ID" do
    test "handler is invoked for registered task ID" do
      ...
      {:ok, ^context} = Bpmn.execute(start, context)
      assert Bpmn.Context.get_data(context, "handled_by") == "specific_task"
    end
    ...
    test "task ID takes priority over type registration" do
      ...
      assert Bpmn.Context.get_data(context, "handled_by") == "priority_task"
      assert Bpmn.Context.get_data(context, "handler_type") == nil
    end
  end
  ...
  describe "unregistered types" do
    test "unregistered types return nil (existing behavior)" do
      ...
      result = Bpmn.execute({:totally_unknown, %{id: "unknown_task"}}, context)
