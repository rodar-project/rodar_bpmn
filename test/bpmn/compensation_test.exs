defmodule Bpmn.CompensationTest do
  ...
  describe "register_handler/3" do
    test "stores a handler entry", %{context: context} do
      Bpmn.Compensation.register_handler(context, "task1", "compensate_task1")
      handlers = Bpmn.Compensation.handlers(context)
    ...
    test "appends multiple handlers", %{context: context} do
      Bpmn.Compensation.register_handler(context, "task1", "comp1")
      Bpmn.Compensation.register_handler(context, "task2", "comp2")

      assert length(Bpmn.Compensation.handlers(context)) == 2
    end
  end

  describe "handlers/1" do
    test "returns empty list when no handlers registered", %{context: context} do
      assert Bpmn.Compensation.handlers(context) == []
    end
  end
  ...
  describe "compensate_activity/2" do
    test "executes specific handler", %{context: context} do
      ...
      Bpmn.Context.swap_process(context, process)

      Bpmn.Compensation.register_handler(context, "task1", "comp_handler")
      result = Bpmn.Compensation.compensate_activity(context, "task1")

      assert {:ok, ^context} = result
    end

    test "returns error for missing handler", %{context: context} do
      assert {:error, msg} = Bpmn.Compensation.compensate_activity(context, "nonexistent")
  ...
  describe "compensate_all/1" do
    test "executes handlers in reverse completion order", %{context: context} do
      ...
      {:ok, _} = Bpmn.Compensation.compensate_all(context)

      # Both handlers executed
      assert Bpmn.Context.get_data(context, :comp1_result) == "compensated_1"
      assert Bpmn.Context.get_data(context, :comp2_result) == "compensated_2"
    ...
    test "returns ok with no handlers", %{context: context} do
      assert {:ok, ^context} = Bpmn.Compensation.compensate_all(context)
    end
  end
end
