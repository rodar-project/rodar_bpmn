defmodule Bpmn.CollaborationTest do
  ...
  describe "start/2" do
    test "starts a two-participant collaboration with independent processes" do
      ...
      assert Bpmn.Process.status(result.instances["ProcessA"]) == :completed
      assert Bpmn.Process.status(result.instances["ProcessB"]) == :completed
    end
  ...
  describe "message flow wiring" do
    test "two-pool collaboration with message throw/catch" do
      ...
      assert Bpmn.Process.status(result.instances["ProcSender"]) == :completed
      ...
      status_b = Bpmn.Process.status(result.instances["ProcReceiver"])
