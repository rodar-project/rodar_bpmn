defmodule Rodar.CompensationTest do
  ...
  describe "register_handler/3" do
    test "stores a handler entry", %{context: context} do
      Rodar.Compensation.register_handler(context, "task1", "compensate_task1")
      handlers = Rodar.Compensation.handlers(context)
    ...
    test "appends multiple handlers", %{context: context} do
      Rodar.Compensation.register_handler(context, "task1", "comp1")
      Rodar.Compensation.register_handler(context, "task2", "comp2")

      assert length(Rodar.Compensation.handlers(context)) == 2
    end
  end

  describe "handlers/1" do
    test "returns empty list when no handlers registered", %{context: context} do
      assert Rodar.Compensation.handlers(context) == []
    end
  end
  ...
  describe "compensate_activity/2" do
    test "executes specific handler", %{context: context} do
      ...
      Rodar.Context.swap_process(context, process)

      Rodar.Compensation.register_handler(context, "task1", "comp_handler")
      result = Rodar.Compensation.compensate_activity(context, "task1")

      assert {:ok, ^context} = result
    end

    test "returns error for missing handler", %{context: context} do
      assert {:error, msg} = Rodar.Compensation.compensate_activity(context, "nonexistent")
  ...
  describe "compensate_all/1" do
    test "executes handlers in reverse completion order", %{context: context} do
      ...
      {:ok, _} = Rodar.Compensation.compensate_all(context)

      # Both handlers executed
      assert Rodar.Context.get_data(context, :comp1_result) == "compensated_1"
      assert Rodar.Context.get_data(context, :comp2_result) == "compensated_2"
    ...
    test "returns ok with no handlers", %{context: context} do
      assert {:ok, ^context} = Rodar.Compensation.compensate_all(context)
    end
  end
end
