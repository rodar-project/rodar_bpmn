defmodule Bpmn.TaskRegistry do
  @moduledoc """
  Registry for custom task handlers.

  Maps task type atoms or task ID strings to handler modules implementing
  the `Bpmn.TaskHandler` behaviour. Lookup priority: task ID (string) first,
  then task type (atom).

  ## Examples

      iex> Bpmn.TaskRegistry.register(:my_task, MyHandler)
      :ok
      iex> {:ok, MyHandler} = Bpmn.TaskRegistry.lookup(:my_task)
      iex> Bpmn.TaskRegistry.unregister(:my_task)
      :ok
