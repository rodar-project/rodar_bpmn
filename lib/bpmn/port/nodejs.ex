defmodule Bpmn.Port.Nodejs do
  ...
  def start_link(cmd \\ nil) do
    cmd =
      case cmd do
        nil -> ["node #{:code.priv_dir(:bpmn)}/scripts/node.js"]
        _ -> cmd
      end
  ...
  def handle_call({:string, script, context}, from, port),
    do: send_request({"string", script, context}, from, port)

  def handle_call({:script, script, context}, from, port),
    do: send_request({"file", script, context}, from, port)
